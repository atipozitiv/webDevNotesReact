...
export default function TaskList({ taskId, tasks, setTaskId }) {
  ...
  const deleteCancel = () =>
    setDeletePopup({ isVisible: false, isConfirm: false });
  ...
  function deleteTask() {
    ...
    while (turn < taskId - 1) {
      tasks.set(turn, {
        id: turn,
        title: tasks.get(turn + 1).title,
        about: tasks.get(turn + 1).about,
      });
      tasks.delete(turn + 1);
      turn += 1;
    }
  ...
  function dragEndHandler(e) {
    e.target.className == "task-block-text"
      ? (e.target.style.background = "#1F1E1B")
      : null;
  }
  ...
  function dragOverHandler(e) {
    e.preventDefault();
    e.target.className == "task-block-text"
      ? (e.target.style.background = "#201f1c")
      : null;
  }
  ...
  function dropHandler(e, task) {
    ...
    tasks.set(task[1].id, {
      id: task[1].id,
      title: draggedTask[1].title,
      about: draggedTask[1].about,
    });
    tasks.set(draggedTask[1].id, {
      id: draggedTask[1].id,
      title: task[1].title,
      about: task[1].about,
    });
    ...
    return (
      <>
        ...
        {deletePopup.isVisible ? (
          <DeletePopup confirm={deleteConfirm} cancel={deleteCancel}></DeletePopup>
        ) : (
          <></>
        )}
      </>
